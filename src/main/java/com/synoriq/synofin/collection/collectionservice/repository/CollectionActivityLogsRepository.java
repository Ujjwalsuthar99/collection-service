...
public interface CollectionActivityLogsRepository extends PagingAndSortingRepository<CollectionActivityLogsEntity, Long> {
    ...
    @Query(nativeQuery = true,value = "select * from collection.collection_activity_logs cal where cal.remarks like '%:serviceRequestId%' and cal.activity_name in ('receipt_approved', 'receipt_rejected')")
    CollectionActivityLogsEntity getActivityLogsKafkaByReceiptId(@Param("loanId") String serviceRequestId);
