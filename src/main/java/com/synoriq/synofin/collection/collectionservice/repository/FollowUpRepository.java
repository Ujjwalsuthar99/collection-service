...
//            "select concat_ws(' ', c.first_name, c.last_name) as name,\n" +
            ...
            "            and f.next_followup_datetime between :fromDate and :toDate and f.followup_status in :#{#statusList} order by\n"+
            "  case when f.followup_status = 'pending' then 1\n" +
            "  when f.followup_status = 'reschedule' then 2\n" +
            "  else 3 end")
