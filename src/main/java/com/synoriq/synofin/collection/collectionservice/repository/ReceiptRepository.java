...
public interface ReceiptRepository extends JpaRepository<FollowUpEntity, Long> {
    ...
    @Query(nativeQuery = true, value = "select \n" +
            ...
            "    and (cr.receipt_holder_user_id = (select user_id from master.users where username = 'col1')\n" +
            "    and cr.receipt_id not in (select rth.collection_receipts_id from collection.receipt_transfer_history rth join receipt_transfer rt on rth.receipt_transfer_id = rt.receipt_transfer_id where rt.status = 'pending'))")
    ...
    @Query(nativeQuery = true, value = "select sr.loan_id as loanId from lms.service_request sr join collection.collection_receipts cr on sr.service_request_id = cr.receipt_id where sr.service_request_id = cast(:serviceRequestId as bigint) and sr.request_source = 'm_collect' and sr.is_deleted = false")
