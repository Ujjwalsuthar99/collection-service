...
public interface ReceiptRepository extends JpaRepository<FollowUpEntity, Long> {
    ...
    @Query(nativeQuery = true, value = "select sr.service_request_id as id, sr.created_date as date from lms.service_request sr join collection.collection_receipts cr on sr.service_request_id = cr.receipt_id where sr.request_source = 'm_collect' and cast(sr.form->>'receipt_amount' as text) = :receiptAmount and sr.loan_id = :loanId and sr.is_deleted = false order by sr.created_date desc limit 1")
