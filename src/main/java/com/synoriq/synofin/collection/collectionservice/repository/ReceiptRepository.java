...
//            "    concat_ws(' ', c.first_name, c.last_name) as customer_name,\n" +
    ...
    @Query(nativeQuery = true, value = "select sr.service_request_id as serviceRequestId from lms.service_request sr join collection.collection_receipts cr on sr.service_request_id = cr.receipt_id where sr.form->>'transaction_reference' = :transactionReferenceNumber and sr.request_source = 'm_collect' and sr.form->>'payment_mode' in ('upi', 'neft', 'rtgs') and sr.status in ('approved', 'initiated') and sr.is_deleted = false")
